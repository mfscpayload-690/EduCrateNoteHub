...
function renderSearchResults(results) {
    const html = results.map(f => {
        ...
        const fileJson = JSON.stringify(f).replace(/'/g, '&#39;');
        return '<div class="search-result cursor-pointer px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-700/70 flex items-center gap-3 border-b border-slate-100 dark:border-slate-700 last:border-0 transition-colors" data-file=\'' + fileJson + '\'>' +
